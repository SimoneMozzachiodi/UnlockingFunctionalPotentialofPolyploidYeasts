...
MyRef="S288c"
sample=#here add all samples used
...
do
   for indsample in $sample
    do
  /opt/tvs/bin/freebayes -f $genomes/$ref.genome.chrref.fa -@ $wd/"OS1364S288c.Filtvarp3Pos.vcf.gz" -p3 -m 30 -q 20 -i -X -u  $InpDir/$indsample/$indsample.srt.rmd.bam > $OutDir/$indsample/$indsample$ref/$indsample$ref.var.p3.2.vcf 
